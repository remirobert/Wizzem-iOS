...
@interface MBProgressHUD () {
    ...
    UIVisualEffectView *roundBg;
    UIVisualEffectView *vibrancyRoundBg;
    UIView *contentView;
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        roundBg = [[UIVisualEffectView alloc] initWithEffect:[UIBlurEffect effectWithStyle:UIBlurEffectStyleExtraLight]];
        roundBg.clipsToBounds = YES;
        roundBg.frame = self.bounds;
        [contentView addSubview:roundBg];
        ...
        vibrancyRoundBg = [[UIVisualEffectView alloc] initWithEffect:vibrancyEffect];
        [vibrancyRoundBg setFrame:self.bounds];
        [[roundBg contentView] addSubview:vibrancyRoundBg];
...
- (void)setupLabels {
	...
	[[vibrancyRoundBg contentView] addSubview:label];
	...
	[[vibrancyRoundBg contentView] addSubview:detailsLabel];
}
...
//			[[vibrancyRoundBg contentView] addSubview:indicator];
            
            self.indicator = [[MMMaterialDesignSpinner alloc] initWithFrame:CGRectZero];
            self.indicator.bounds = CGRectMake(0, 0, 40, 40);
            self.indicator.tintColor = [UIColor blackColor];
            self.indicator.center = CGPointMake(CGRectGetMidX(vibrancyRoundBg.bounds), CGRectGetMidY(vibrancyRoundBg.bounds));
            self.indicator.translatesAutoresizingMaskIntoConstraints = NO;
            [[vibrancyRoundBg contentView] addSubview:self.indicator];
            [self.indicator performSelector:@selector(startAnimating)];
		}
...
- (void)layoutSubviews {
    ...
    contentView.frame = boxRect;
    boxRect.origin.x *= -1;
    boxRect.origin.y *= -1;
    boxRect.size.width = roundBg.frame.size.width;
    boxRect.size.height = roundBg.frame.size.height;
    roundBg.frame = boxRect;
}
